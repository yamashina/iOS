...
- (void)viewDidLoad {
	...
	// ＋ボタン生成(押下でイベント追加画面に遷移させる)
//  UIBarButtonItem *addButton = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemAdd target:self action:@selector(insertNewObject)];
    UIBarButtonItem *addButton = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemAdd 
																			   target:self 
																			   action:@selector(moveToAddView)];
    self.navigationItem.rightBarButtonItem = addButton;
    [addButton release];
	
	// ナビゲーションバータイトル
	CountDownTimeTableAppDelegate *appDelegate = [[UIApplication sharedApplication] delegate];
	RootViewController *viewController = (RootViewController *)appDelegate.navigationController.topViewController;
	viewController.title = @"Rest --:--:--";
	
	// ターゲット行(現在時刻から一番近いイベント)の決定
	[self decideTargetEvent];
}
...
- (void)moveToAddView {
	AddViewController *detailViewController = [[AddViewController alloc]initWithNibName:@"AddViewController" bundle:nil];
...
- (void)decideTargetEvent {
	...
	for (int i=0; i<[self.tableView numberOfRowsInSection:0]; i++) {
		...
		NSDate *targetTime = [managedObject valueForKey:@"time"];
		
		NSDateFormatter *fmt = [[NSDateFormatter alloc] init];
		[fmt setDateFormat:@"yyyy/MM/dd HH:mm:ss"];
		NSDate *nowDate = [NSDate date];
		NSString *nowTimeStr = [NSString 
								stringWithFormat:@"2000/01/01 %02d:%02d:00", 
								[self getHourFromDate:nowDate], 
								[self getMinuteFromDate:nowDate]];
		NSDate *nowTime = [fmt dateFromString:nowTimeStr];
		[fmt release];
...
// 詳細ディスクロージャーボタンのタップイベントハンドラ
- (void)tableView:(UITableView *)tableView accessoryButtonTappedForRowWithIndexPath:(NSIndexPath *)indexPath {
	NSManagedObject *selectedObject = [[self fetchedResultsController] objectAtIndexPath:indexPath];
	[self moveToEditView:selectedObject];
}
